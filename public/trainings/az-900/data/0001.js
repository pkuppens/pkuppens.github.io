...
window.TRAINING_DATA['0001'] = {
  ...
  questions: [
    { tag:"D1",
      q:"In Azure's shared responsibility model, who is always responsible for data classification and endpoint protection, regardless of IaaS/PaaS/SaaS?",
      options:["The customer","Microsoft","Whichever party owns the datacenter","No one — it's automated"],
      answer:0,
      why:"<b>The customer</b> always owns data, endpoints, and access management — this doesn't shift with service model. Microsoft's responsibility grows (physical → OS → app) as you move IaaS → PaaS → SaaS." },
    ...
    { tag:"D2",
      q:"Physically separate datacenters within an Azure region, each with independent power/cooling/networking, are called a(n):",
      options:["Availability zone","Region pair","Resource group","Management group"],
      answer:0,
      why:"Azure calls this an <b>availability zone</b>. A region pair is a DR concept (two regions in the same geography); resource groups and management groups are organizational containers, not physical infrastructure." },
    ...
    { tag:"D2",
      q:"Azure's construct for applying policies and RBAC across MULTIPLE subscriptions at once is:",
      options:["A management group","A resource group","A tenant","An availability set"],
      answer:0,
      why:"<b>Management groups</b> sit above subscriptions and let you govern many subscriptions at once. A resource group scopes resources within one subscription; a tenant is your Entra ID directory; an availability set is a VM fault-domain construct, unrelated." },
    ...
    { tag:"D2",
      q:"Which Azure compute option lets you run identical VM instances that automatically scale out/in based on demand or a schedule?",
      options:["Azure Virtual Machine Scale Sets","Azure Virtual Desktop","An availability set","Azure Container Instances"],
      answer:0,
      why:"Auto-scaling groups of identical VMs is <b>Virtual Machine Scale Sets</b>. Availability sets group VMs for fault-domain redundancy but don't auto-scale; Virtual Desktop is a VDI service; Container Instances runs containers, not VMs." },

    { tag:"D2",
      q:"Which Azure Storage redundancy option keeps synchronous copies of your data across three availability zones within one region?",
    ...
    { tag:"D2",
      q:"Which Azure service is the identity directory that manages users, groups, and app registrations for sign-in and access?",
      options:["Microsoft Entra ID","Azure Resource Manager","Azure Policy","Azure Key Vault"],
      answer:0,
      why:"<b>Microsoft Entra ID</b> (formerly Azure Active Directory) is the identity directory, though Azure splits identity (Entra ID) from permission assignment (RBAC) rather than combining both under one umbrella." },

    { tag:"D3",
      q:"Azure's mechanism for granting a user, group, or app a specific set of permissions at a particular scope (management group/subscription/resource group/resource) is:",
      options:["Azure role-based access control (RBAC)","Azure Policy","A resource lock","Microsoft Purview"],
      answer:0,
      why:"Assigning permissions at a scope is <b>RBAC</b>, layered on Entra ID identities. Azure Policy enforces configuration rules (not permissions); resource locks prevent delete/edit; Purview is for data governance/cataloging." },
